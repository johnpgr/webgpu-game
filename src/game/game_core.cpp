...
GAME_DLL_EXPORT void game_update(GameState* game, f64 dt) {
    ...
    u32 bone_frame_count = atlas_animation_frame_count(WEAPONS_BONE_BONE);
    u32 bonfire_frame_count = atlas_animation_frame_count(
        ENVIRONMENT_STRUCTURES_STATIONS_BONFIRE_BONFIRE_01
    );

    if(bone_frame_count > 0) {
        AtlasFrame bone_frame = atlas_animation_frame(WEAPONS_BONE_BONE, 0);
        if(bone_frame.width_px > 0.0f && bone_frame.height_px > 0.0f) {
            push_world_sprite(
    ...
    if(bonfire_frame_count > 0) {
        u32 frame_index = (u32)(game->time * 8.0) % bonfire_frame_count;
        AtlasFrame bonfire_frame = atlas_animation_frame(
            ENVIRONMENT_STRUCTURES_STATIONS_BONFIRE_BONFIRE_01,
            frame_index
        );
        if(bonfire_frame.width_px > 0.0f && bonfire_frame.height_px > 0.0f) {
            push_world_sprite(
                &game->render_frame.world_sprites,
